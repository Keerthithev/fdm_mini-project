...
def generate_pdf_report(predictions_df: pd.DataFrame, model_info: str = "Fraud Detection Model") -> bytes:
    """Generate an attractive PDF report for batch predictions."""
    buffer = io.BytesIO()
    ...
    story.append(Paragraph(f"Model: {model_info}", styles['Normal']))
    story.append(Paragraph(f"Total Transactions Analyzed: {len(predictions_df)}", styles['Normal']))
    story.append(Spacer(1, 20))
    
    # Summary statistics
    if 'prediction' in predictions_df.columns:
        fraud_count = (predictions_df['prediction'] == 1).sum()
        non_fraud_count = (predictions_df['prediction'] == 0).sum()
        fraud_rate = (fraud_count / len(predictions_df)) * 100
        
        story.append(Paragraph("Summary Statistics", heading_style))
        story.append(Paragraph(f"• Fraudulent Transactions: {fraud_count} ({fraud_rate:.1f}%)", styles['Normal']))
        story.append(Paragraph(f"• Non-Fraudulent Transactions: {non_fraud_count} ({100-fraud_rate:.1f}%)", styles['Normal']))
        story.append(Spacer(1, 20))
    
    # Sample predictions table
    story.append(Paragraph("Sample Predictions", heading_style))
    
    # Prepare table data
    display_df = predictions_df.head(20).copy()  # Show first 20 rows
    
    # Format the dataframe for PDF
    table_data = [list(display_df.columns)]
    for _, row in display_df.iterrows():
        formatted_row = []
        for col in display_df.columns:
            if col == 'prediction':
                formatted_row.append('FRAUD' if row[col] == 1 else 'NON-FRAUD')
            elif col == 'probability' and pd.notna(row[col]):
    ...
    # Footer
    story.append(Paragraph("This report was generated by the Fraud Detection System", styles['Normal']))
    story.append(Paragraph("For questions or concerns, please contact the fraud analysis team.", styles['Normal']))
...
def render_batch_prediction(model, df: Optional[pd.DataFrame]):
    ...
    st.markdown("Upload a CSV file to analyze multiple transactions at once. The system will classify each transaction as FRAUD or NON-FRAUD.")
    ...
    upload = st.file_uploader("Choose a CSV file", type=["csv"], help="Upload a CSV file containing transaction data")
    
    if upload is not None:
        pred_df = safe_read_csv(upload)
        
        if pred_df is None or pred_df.empty:
            st.error("Could not read the uploaded file. Please check the file format.")
            return
        
        st.success(f"✅ Successfully loaded {len(pred_df)} transactions")
        
        # Show data preview
        with st.expander("📋 Data Preview", expanded=True):
            st.dataframe(pred_df.head(10))
            st.caption(f"Total rows: {len(pred_df)}, Columns: {list(pred_df.columns)}")
        ...
        if not feature_cols:
            st.info("Select at least one feature column to proceed.")
            return
        ...
        if st.button("🔍 Analyze Transactions", type="primary"):
            with st.spinner("Analyzing transactions..."):
                ...
                out["prediction_label"] = out["prediction"].map({0: "NON-FRAUD", 1: "FRAUD"})
                
                if proba is not None and proba.ndim == 2 and proba.shape[1] >= 2:
                    out["fraud_probability"] = proba[:, 1]
                    out["confidence"] = out["fraud_probability"].apply(lambda x: max(x, 1-x))

                # Display results
                st.subheader("🎯 Analysis Results")
                
                # Summary statistics
                fraud_count = (out["prediction"] == 1).sum()
                non_fraud_count = (out["prediction"] == 0).sum()
                fraud_rate = (fraud_count / len(out)) * 100
                
                col1, col2, col3, col4 = st.columns(4)
                with col1:
                    st.metric("Total Transactions", len(out))
                with col2:
                    st.metric("Fraudulent", fraud_count, f"{fraud_rate:.1f}%")
                with col3:
                    st.metric("Legitimate", non_fraud_count, f"{100-fraud_rate:.1f}%")
                with col4:
                    if "fraud_probability" in out.columns:
                        avg_confidence = out["confidence"].mean()
                ...
                with col1:
                    fraud_dist = out["prediction_label"].value_counts()
                    st.bar_chart(fraud_dist)
                
                with col2:
                    if "fraud_probability" in out.columns:
                        # Risk distribution
                        risk_bins = pd.cut(out["fraud_probability"], bins=5, labels=['Very Low', 'Low', 'Medium', 'High', 'Very High'])
                ...
                display_cols = ["prediction", "prediction_label"]
                if "fraud_probability" in out.columns:
                    display_cols.append("fraud_probability")
                if "confidence" in out.columns:
                    display_cols.append("confidence")
                
                # Add some original columns for context
                context_cols = [col for col in pred_df.columns if col in out.columns][:3]  # First 3 original columns
                display_cols = context_cols + display_cols
                
                styled_df = out[display_cols].style.apply(highlight_fraud, axis=1)
                ...
                with col2:
                    # PDF download
                    try:
                        pdf_data = generate_pdf_report(out, "Fraud Detection Model")
                        st.download_button(
                            "📊 Download PDF Report", 
                            data=pdf_data, 
                            file_name=f"fraud_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf", 
                            mime="application/pdf",
                            help="Download an attractive PDF report with analysis summary"
                        )
                ...
                if fraud_count > 0:
                    ...
                    fraud_transactions = out[out["prediction"] == 1]
                    
                    for idx, row in fraud_transactions.head(5).iterrows():  # Show top 5 fraud cases
                        with st.expander(f"🚨 Fraud Alert - Transaction {idx + 1}", expanded=False):
                            st.write(f"**Fraud Probability:** {row.get('fraud_probability', 'N/A'):.1%}" if 'fraud_probability' in row else "**Classification:** FRAUD")
                            st.write(f"**Confidence:** {row.get('confidence', 'N/A'):.1%}" if 'confidence' in row else "")
                            
                            # Show key features that might indicate fraud
                ...
                # Success message
                st.success(f"✅ Analysis complete! Processed {len(out)} transactions with {fraud_count} flagged as fraudulent.")
        ...
        with st.expander("📝 Expected Data Format", expanded=False):
            st.markdown("""
            Your CSV file should contain transaction data with columns such as:
            - Transaction_Amount
            - Account_Balance  
            - Risk_Score
            ...
            **Note:** The system will automatically detect and use numeric columns for prediction.
            """)
...
def render_ai_analysis(model, reference_df: Optional[pd.DataFrame]):
    """AI Analysis chatbot that asks questions one by one to gather transaction details."""
    st.header("🤖 AI Fraud Analysis Assistant")
    st.markdown("I'll help you analyze a transaction step by step. Let me ask you some questions to understand the transaction details.")
    ...
    if "ai_analysis_step" not in st.session_state:
        st.session_state["ai_analysis_step"] = 0
    ...
    questions = [
        {
            ...
            "min": 0.0,
            "step": 0.01,
            "help": "Enter the amount of money involved in this transaction"
        },
        {
            ...
            "min": 0.0,
            "step": 1.0,
            "help": "Enter the account balance before this transaction"
        },
        {
            "question": "What is the risk score for this transaction? (0-1 scale)",
            "key": "Risk_Score",
            "type": "slider",
            "min": 0.0,
            "max": 1.0,
            "step": 0.01,
            "help": "Risk score based on previous analysis (0 = low risk, 1 = high risk)"
        },
        {
            "question": "How far is the transaction location from the account holder's usual location? (in km)",
            "key": "Transaction_Distance",
            "type": "number",
            "min": 0.0,
            "step": 0.1,
            "help": "Distance between transaction location and usual location"
        },
        {
            "question": "What is the average transaction amount in the last 7 days?",
            "key": "Avg_Transaction_Amount_7d",
            "type": "number",
            "min": 0.0,
            "step": 1.0,
            "help": "Average amount of transactions in the past week"
        },
        {
            "question": "How many transactions were made today?",
            "key": "Daily_Transaction_Count",
            "type": "number",
            "min": 0,
            "step": 1,
            "help": "Total number of transactions made today"
        },
        {
            "question": "How many failed transactions occurred in the last 7 days?",
            "key": "Failed_Transaction_Count_7d",
            "type": "number",
            "min": 0,
            "step": 1,
            "help": "Number of failed or declined transactions in the past week"
        },
        {
            "question": "How old is the card in days?",
            "key": "Card_Age",
            "type": "number",
            "min": 0,
            "step": 1,
            "help": "Age of the payment card in days"
        },
        {
            "question": "Has there been any previous fraudulent activity on this account?",
            "key": "Previous_Fraudulent_Activity",
            "type": "boolean",
            "help": "Whether any fraudulent activity was detected before"
        },
        {
            "question": "Is this transaction happening on a weekend?",
            "key": "Is_Weekend",
            "type": "boolean",
            "help": "Whether the transaction is occurring on Saturday or Sunday"
        },
        {
            "question": "What type of transaction is this?",
            "key": "Transaction_Type",
            "type": "select",
            "options": ["Online", "POS", "ATM", "Other"],
            "help": "The method or channel used for the transaction"
        },
        {
            "question": "What type of device is being used?",
            "key": "Device_Type",
            "type": "select",
            "options": ["Mobile", "Desktop", "Tablet", "Other"],
            "help": "The device used to initiate the transaction"
        },
        {
            "question": "What is the merchant category?",
            "key": "Merchant_Category",
            "type": "select",
            "options": ["Electronics", "Grocery", "Restaurants", "Travel", "Gas", "Other"],
            "help": "The category of the merchant or business"
        },
        {
            "question": "What type of card is being used?",
            "key": "Card_Type",
            "type": "select",
            "options": ["Debit", "Credit", "Prepaid", "Other"],
            "help": "The type of payment card"
        },
        {
            "question": "What authentication method is being used?",
            "key": "Authentication_Method",
            "type": "select",
            "options": ["PIN", "OTP", "Biometric", "Other"],
            "help": "The method used to authenticate the transaction"
        }
    ]
    
    current_step = st.session_state["ai_analysis_step"]
    
    if current_step < len(questions):
        q = questions[current_step]
        
        st.subheader(f"Question {current_step + 1} of {len(questions)}")
        ...
        if q["type"] == "number":
            value = st.number_input(
                ...
                step=q.get("step", 1),
                key=f"ai_input_{q['key']}"
            )
        elif q["type"] == "slider":
            value = st.slider(
                "Your answer:",
                min_value=q.get("min", 0.0),
                max_value=q.get("max", 1.0),
                step=q.get("step", 0.01),
                key=f"ai_input_{q['key']}"
            )
        elif q["type"] == "boolean":
            value = st.radio(
        ...
        with col1:
            if st.button("Next Question", type="primary"):
                st.session_state["ai_analysis_data"][q["key"]] = value
                st.session_state["ai_analysis_step"] += 1
                st.rerun()
        
        with col2:
            if st.button("Previous Question") and current_step > 0:
                st.session_state["ai_analysis_step"] -= 1
                st.rerun()
    
    else:
        # All questions answered, show analysis
        st.success("✅ All questions completed! Let me analyze this transaction...")
        
        # Prepare data for prediction
        inputs = st.session_state["ai_analysis_data"].copy()
        
        # Convert categorical variables to one-hot encoding
        one_hot_mapping = {
            "Transaction_Type": {
                "Online": {"Transaction_Type_Online": 1, "Transaction_Type_POS": 0},
                "POS": {"Transaction_Type_Online": 0, "Transaction_Type_POS": 1},
                "ATM": {"Transaction_Type_Online": 0, "Transaction_Type_POS": 0},
                "Other": {"Transaction_Type_Online": 0, "Transaction_Type_POS": 0}
            },
            "Device_Type": {
                "Mobile": {"Device_Type_Mobile": 1, "Device_Type_Tablet": 0},
                "Tablet": {"Device_Type_Mobile": 0, "Device_Type_Tablet": 1},
                "Desktop": {"Device_Type_Mobile": 0, "Device_Type_Tablet": 0},
                "Other": {"Device_Type_Mobile": 0, "Device_Type_Tablet": 0}
            },
            "Merchant_Category": {
                "Electronics": {"Merchant_Category_Electronics": 1, "Merchant_Category_Grocery": 0, "Merchant_Category_Restaurants": 0, "Merchant_Category_Travel": 0},
                "Grocery": {"Merchant_Category_Electronics": 0, "Merchant_Category_Grocery": 1, "Merchant_Category_Restaurants": 0, "Merchant_Category_Travel": 0},
                "Restaurants": {"Merchant_Category_Electronics": 0, "Merchant_Category_Grocery": 0, "Merchant_Category_Restaurants": 1, "Merchant_Category_Travel": 0},
                "Travel": {"Merchant_Category_Electronics": 0, "Merchant_Category_Grocery": 0, "Merchant_Category_Restaurants": 0, "Merchant_Category_Travel": 1},
                "Gas": {"Merchant_Category_Electronics": 0, "Merchant_Category_Grocery": 0, "Merchant_Category_Restaurants": 0, "Merchant_Category_Travel": 0},
                "Other": {"Merchant_Category_Electronics": 0, "Merchant_Category_Grocery": 0, "Merchant_Category_Restaurants": 0, "Merchant_Category_Travel": 0}
            },
            "Card_Type": {
                "Debit": {"Card_Type_Debit": 1, "Card_Type_Prepaid": 0},
                "Prepaid": {"Card_Type_Debit": 0, "Card_Type_Prepaid": 1},
                "Credit": {"Card_Type_Debit": 0, "Card_Type_Prepaid": 0},
                "Other": {"Card_Type_Debit": 0, "Card_Type_Prepaid": 0}
            },
            "Authentication_Method": {
                "OTP": {"Authentication_Method_OTP": 1, "Authentication_Method_PIN": 0},
                "PIN": {"Authentication_Method_OTP": 0, "Authentication_Method_PIN": 1},
                "Biometric": {"Authentication_Method_OTP": 0, "Authentication_Method_PIN": 0},
                "Other": {"Authentication_Method_OTP": 0, "Authentication_Method_PIN": 0}
            }
        }
        
        # Apply one-hot encoding
        for cat_var, mapping in one_hot_mapping.items():
            if cat_var in inputs:
                value = inputs[cat_var]
                if value in mapping:
                    inputs.update(mapping[value])
                del inputs[cat_var]
        ...
        try:
            ...
            if prob1 is not None:
                decision = "FRAUD" if prob1 >= 0.5 else "NON-FRAUD"
                confidence = prob1 if decision == "FRAUD" else (1 - prob1)
            else:
                decision = "FRAUD" if pred_class == 1 else "NON-FRAUD"
            ...
            # Result display
            if decision == "FRAUD":
                st.error(f"🚨 **FRAUD DETECTED** - Confidence: {confidence:.1%}")
            else:
                st.success(f"✅ **LEGITIMATE TRANSACTION** - Confidence: {confidence:.1%}")
            
            if prob1 is not None:
                st.metric("Fraud Probability", f"{prob1:.1%}")
                st.progress(prob1)
            
            # AI Analysis summary
            st.subheader("🤖 AI Analysis Summary")
            
            # Generate insights based on the inputs
            insights = []
            
            if inputs.get("Transaction_Amount", 0) > 1000:
                insights.append("💰 High transaction amount detected")
            if inputs.get("Risk_Score", 0) > 0.7:
                insights.append("⚠️ High risk score indicates suspicious activity")
            if inputs.get("Transaction_Distance", 0) > 100:
                insights.append("🌍 Transaction location is far from usual location")
            if inputs.get("Daily_Transaction_Count", 0) > 10:
                insights.append("📊 Unusually high number of transactions today")
            if inputs.get("Failed_Transaction_Count_7d", 0) > 2:
                insights.append("❌ Multiple failed transactions in recent history")
            if inputs.get("Previous_Fraudulent_Activity", 0) == 1:
                insights.append("🚨 Previous fraudulent activity on this account")
            if inputs.get("Is_Weekend", 0) == 1:
                insights.append("📅 Weekend transaction (higher risk period)")
            ...
            if insights:
                ...
                st.info("No significant risk factors identified in this transaction.")
            ...
            if decision == "FRAUD":
                ...
                **Immediate Actions:**
                ...
                • Contact the account holder for verification
...
def main():
    ...
    # Sidebar: navigation
    st.sidebar.header("🎯 Fraud Detection System")
    st.sidebar.markdown("Choose your analysis method:")
    
    page = st.sidebar.radio(
        "Select Analysis Type", 
        ("📊 Batch Prediction", "🔍 Single Prediction", "🤖 AI Analysis"),
        index=0
    )
    ...
    if df is not None and not df.empty:
        ...
        st.sidebar.warning("📁 No Sample Data")
    ...
    if page == "📊 Batch Prediction":
        render_batch_prediction(model, df)
    elif page == "🔍 Single Prediction":
        render_single_prediction(model, df)
    elif page == "🤖 AI Analysis":
        render_ai_analysis(model, df)
